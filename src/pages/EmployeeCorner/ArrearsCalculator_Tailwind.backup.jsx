...
function ArrearsCalculator() {
    ...
    const renderComponentInputs = (title, type, compKey, label) => {
        ...
        return (
            <div className={`mb-3 bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden transition-all hover:shadow-md ${borderColorClass}`}>
                <div className="px-3 py-1.5 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                    <span className={`font-bold text-[10px] uppercase tracking-wider ${headerColorClass}`}>{label}</span>
                    {toggles.promotionEnabled && (
                        <button className="text-[10px] bg-white border border-slate-200 text-slate-500 px-1.5 py-0.5 rounded shadow-sm hover:text-blue-600 hover:border-blue-300 transition-colors" onClick={() => alert('Feature: Split Period')}>
                            + Split
                        </button>
                    )}
                </div>
                    ...
                    {comps.map((item, idx) => (
                        <div key={idx} className="flex gap-2 items-center">
                            <div className="relative flex-grow">
                                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] font-medium">₹</span>
                                <input
                                    type="number"
                                    value={item.amount}
                                    onChange={(e) => updateComponent(type, compKey, idx, 'amount', e.target.value)}
                                    className="w-full pl-5 pr-2 py-1.5 text-xs font-bold text-slate-700 placeholder-slate-400 bg-white border border-slate-200 rounded focus:outline-none focus:ring-1 focus:ring-offset-0 focus:ring-indigo-500 transition-all"
                                    placeholder="0"
                                />
                            </div>
                            {toggles.promotionEnabled && (
                                <>
                                    <div className="w-32">
                                        <DatePicker
                                            ...
                                            dateFormat="yyyy-MM"
                                            showMonthYearPicker
                                            className="w-full text-xs font-medium text-slate-600 border border-slate-200 rounded-md px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
    ...
    return (
        <div className="animate-fade-in min-h-screen bg-slate-50 py-6 px-4 font-sans text-slate-800 max-w-7xl mx-auto">
            {/* --- MAIN HEADER & CONFIGURATION CARD --- */}
            <div className="w-full bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden mb-6">
                {/* Header */}
                <div className="px-5 py-4 border-b border-slate-100 bg-gradient-to-br from-white to-slate-50 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="bg-blue-600 p-2 rounded-lg shadow-md shadow-blue-200">
                            <span className="text-xl text-white">🏛️</span>
                        </div>
                        <div>
                            <h1 className="text-xl font-extrabold text-slate-800 tracking-tight">Arrears Calculator</h1>
                            <p className="text-xs font-semibold text-slate-500">Government Salary Due-Drawn Statement Generator</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-1">
                            <span className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded text-[10px] font-bold">01</span>
                            <h3 className="font-bold text-slate-700 uppercase tracking-wider text-xs">Employee Details</h3>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Name</label>
                                <input type="text" className="w-full text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">From Month</label>
                                <DatePicker
                                    ...
                                    className="w-full text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm outline-none"
                                    wrapperClassName="w-full"
                                    placeholderText="Select Month"
                                />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">To Month</label>
                                <DatePicker
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">GR / Order Title</label>
                            <textarea className="w-full text-[10px] font-medium text-slate-600 bg-white border border-slate-200 rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm h-12 resize-none"
                    ...
                    <div className="space-y-4 border-l border-slate-100 pl-6">
                        <div className="flex items-center gap-2 mb-1">
                            <span className="bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded text-[10px] font-bold">02</span>
                            <h3 className="font-bold text-slate-700 uppercase tracking-wider text-xs">Configuration</h3>
                        </div>

                        {/* Toggles Panel */}
                        <div className="bg-slate-50 rounded-lg p-3 border border-slate-100 space-y-2">
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] font-bold text-slate-600">Auto-DA (Maharashtra)</span>
                            ...
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] font-bold text-slate-600">Auto-HRA (7th PC)</span>
                                <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                                ...
                                <select className="w-full text-[10px] font-bold text-slate-700 bg-white border border-slate-200 rounded px-2 py-1.5 focus:ring-1 focus:ring-blue-500" value={basicInfo.category} onChange={e => updateBasicInfo('category', e.target.value)}>
                            ...
                            <div className="space-y-1">
                                ...
                                <select className="w-full text-[10px] font-bold text-slate-700 bg-white border border-slate-200 rounded px-2 py-1.5 focus:ring-1 focus:ring-blue-500" value={basicInfo.cityCategory} onChange={e => updateBasicInfo('cityCategory', e.target.value)}>
                            ...
                            <div className="space-y-1 col-span-2">
                                ...
                                <select className="w-full text-[10px] font-bold text-slate-700 bg-white border border-slate-200 rounded px-2 py-1.5 focus:ring-1 focus:ring-blue-500" value={basicInfo.incrementMonth} onChange={e => updateBasicInfo('incrementMonth', e.target.value)}>
                        ...
                        <div className="pt-3 border-t border-slate-100">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block mb-1.5">Custom Columns</label>
                            <div className="flex gap-2 mb-1.5">
                                <input type="text" id="newColInput" className="w-full text-[10px] border border-slate-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500" placeholder="Add column..." />
                                <button onClick={() => { const el = document.getElementById('newColInput'); if (el.value) { addCustomColumn(el.value); el.value = ''; } }}
                                    className="bg-slate-800 text-white text-[10px] font-bold px-2 py-1 rounded hover:bg-slate-900 transition-colors">+</button>
                            </div>
                            <div className="flex flex-wrap gap-1.5">
                                {customColumns.map(col => (
                                    <span key={col.id} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded border border-slate-200">
                                        {col.label}
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden flex flex-col h-full">
                    {/* Header */}
                    <div className="px-4 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 border-b border-emerald-700 flex justify-between items-center shadow-sm z-10">
                        <div className="flex items-center gap-2">
                            <div className="bg-white/20 p-1.5 rounded backdrop-blur-sm">
                                <span className="text-sm">💰</span>
                            </div>
                            <div>
                                <h2 className="text-sm font-bold text-white tracking-wide">DUE AMOUNT</h2>
                                <p className="text-[9px] font-medium text-emerald-100 uppercase tracking-widest opacity-90">सध्या दिले जाणारे वेतन</p>
                            </div>
                        </div>
                    </div>

                    <div className="p-4 bg-slate-50/50 flex-grow text-xs">
                                        ...
                                        duePromotionPeriods.map((period, index) => (
                                            <div key={index} className="bg-white p-4 rounded-xl border border-emerald-200 shadow-emerald-100/50 shadow-md relative group transition-all hover:shadow-lg">
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pl-3">
                                                    <div>
                                                        <label className="text-xs font-bold text-slate-400 uppercase block mb-1">From Date</label>
                                                        <DatePicker
                                                            selected={period.from ? new Date(period.from) : null}
                                                            onChange={(date) => {
                                                            ...
                                                            dateFormat="yyyy-MM-dd"
                                                            className="w-full text-xs font-bold text-slate-700 border border-slate-200 rounded px-2 py-1.5 focus:ring-1 focus:ring-emerald-500 outline-none"
                                                            wrapperClassName="w-full"
                                                            placeholderText="Select Date"
                                                        />
                                                    </div>
                                                    <div>
                                                        <label className="text-xs font-bold text-slate-400 uppercase block mb-1">Basic Pay</label>
                                                    ...
                                                    <div>
                                                        <label className="text-xs font-bold text-slate-400 uppercase block mb-1">DA %</label>
                                                    ...
                                                    <div>
                                                        <label className="text-xs font-bold text-slate-400 uppercase block mb-1">HRA %</label>
                                                    ...
                                                    <div>
                                                        <label className="text-xs font-bold text-slate-400 uppercase block mb-1">TA</label>
                                                    ...
                                                    {customColumns.map(col => (
                                                        <div key={col.id}>
                                                            <label className="text-xs font-bold text-slate-400 uppercase block mb-1">{col.label}</label>
                ...
                <div className="bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden flex flex-col h-full">
                    {/* Header */}
                    <div className="px-4 py-3 bg-gradient-to-r from-orange-600 to-red-600 border-b border-orange-700 flex justify-between items-center shadow-sm z-10">
                        <div className="flex items-center gap-2">
                            <div className="bg-white/20 p-1.5 rounded backdrop-blur-sm">
                                <span className="text-sm">📉</span>
                            </div>
                            <div>
                                <h2 className="text-sm font-bold text-white tracking-wide">DRAWN AMOUNT</h2>
                                <p className="text-[9px] font-medium text-orange-100 uppercase tracking-widest opacity-90">पूर्वी दिलेले वेतन</p>
                            </div>
                        </div>
                    </div>
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pl-3">
                                                    <div>
                                                        ...
                                                        <DatePicker
                                                            ...
                                                            dateFormat="yyyy-MM-dd"
                                                            className="w-full text-xs font-bold text-slate-700 border border-slate-200 rounded px-2 py-1.5 focus:ring-1 focus:ring-orange-500 outline-none"
                                                            wrapperClassName="w-full"
                                                            placeholderText="Select Date"
                                                        />
                                                    </div>
